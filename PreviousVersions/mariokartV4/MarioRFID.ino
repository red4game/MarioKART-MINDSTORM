...
const char* ssid = "ESP32AP";  // ssid (exemple)
const char* password = "esp32wroom";  // Mot de passe (exemple)
  ...
  <footer class="footer">
    <div class="infos">
      ...
      <div class="rightFooter"> 
        <p class="rfid">RFID value : <span id="rfid">%RFID%</span> </p>
      </div>
    </div>
    </footer>
</body>
...
function setupCanvas() {
  ...
  function onMessage(event) {
    ...
    document.getElementById('lap').innerHTML = myMessageContent[0];
    document.getElementById('checkpoint').innerHTML = myMessageContent[1];
    document.getElementById('rfid').innerHTML = myMessageContent[2];
  }
...
void notifyClients() {
  ws.textAll((String)LapNB+"-"+(String)checkPointNB+"-"+((const __FlashStringHelper*) RFIDvalue));
}
...
String processor(const String& var){
  ...
  if (var == "RFID"){
    return ((const __FlashStringHelper*) RFIDvalue);
    //return String(RFIDvalue);
  }
  if (var =="LAP"){
    return ((const __FlashStringHelper*)LapNB);
  }
  if (var =="CHECKPOINT"){
    return ((const __FlashStringHelper*)checkPointNB);
  }
  if (var =="IPSTACK"){
    return ((const __FlashStringHelper*)IPADRESS.toString().c_str());
  }
}


void broadcastIPS(IPAddress broad,IPAddress ip){
  String serverPath;
  ...
  for (int i =1;i<255;i++){
    if (i!=ip[3] or 1==1){
      broad[3] = i;
      serverPath = "http://"+broad.toString()+"/broadM5/";
      http.begin(serverPath.c_str());
      httpResponseCode = http.GET();
      if(httpResponseCode==200){
        payload = http.getString();
        Serial.println("Mon Copain");
        Serial.println(payload);
        http.end();
      } else {
        Serial.println("Pas Mon Copain");
...
void setup() {
  ...
  if(udp.listen(42424)) {
      udp.onPacket([](AsyncUDPPacket packet) {
          if (strncmp((char*)packet.data(), sender, strlen(sender)) == 0){
             udp.print("Its Me Mario!");
          } else if (strncmp((char*)packet.data(), receiver, strlen(receiver)) == 0){
            Serial.println("Copain");
            Serial.println(packet.remoteIP());
            Serial.println((char*)packet.data());
          } else {
            Serial.println("Pas Copain");
            Serial.println(packet.remoteIP());
            Serial.println((char*)packet.data());
          }
    });
  ...
  */
  ...
  server.on("/broadM5/", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/html", BROADSYNC, processor);
  });

  //print du QRcode de wifi
  //M5.Lcd.qrcode("WIFI:S:MONSUPERARDUINO;T:WPA;P:PASSWORD1234;;",0,0,200,6);
  //M5.Lcd.println(IP);
  strcpy(lastRFID,"FFFFFFFFFFFFFFFFFFFF");
  // Start server
  server.begin();
...
void string2hexString(char* input, char* output)
{
    ...
    while(input[loop] != '\0')
    {
        buf += String(input[loop], HEX);
        loop+=1;
    }
    //insert NULL at the end of the output string
    buf.toCharArray(output,21);   
}
...
void loop() {
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial() ) {
    ...
    string2hexString((char*)mfrc522.uid.uidByte,(char*)RFIDvalue);
    if (strcmp(RFIDvalue,lastRFID) != 0) {
      notifyClients();
      Serial.println(((const __FlashStringHelper*) lastRFID));
      strncpy(lastRFID, RFIDvalue,20*sizeof(char));
      //Serial.println(((const __FlashStringHelper*) lastRFID));
    }
    Serial.println(((const __FlashStringHelper*) RFIDvalue));
  }
